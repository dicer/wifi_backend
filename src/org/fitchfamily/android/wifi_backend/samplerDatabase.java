...
public class samplerDatabase {
    ...
    private samplerDatabase(Context context) {
        if (configuration.debug >= configuration.DEBUG_SPARSE) Log.i(TAG, "samplerDatabase.samplerDatabase()");
...
//             jFile.delete();
    ...
    private void setupDatabase(File dbFile) {
        ...
        if (configuration.debug >= configuration.DEBUG_SPARSE) Log.i(TAG, "setupDatabase() version was " + curVer);
    ...
    public void addSample( String bssid, String ssid, String capabilities, Location sampleLoc ) {
        ...
        publishSize();
        if (configuration.debug >= configuration.DEBUG_VERBOSE) Log.i(TAG,"addSample time: "+(System.currentTimeMillis()-entryTime)+"ms");
    }

    public void dropAP( String bssid ) {
        final String canonicalBSSID = bssid.replace(":","");
        if (configuration.debug >= configuration.DEBUG_SPARSE) Log.i(TAG, "Dropping " + canonicalBSSID + " from db" );
    ...
    private void updateAP(apInfo curInfo, String ssid, String capabilities, Location sampleLoc ) {
        ...
        if (diff >= configuration.apMovedThreshold) {
            ...
            bestAP.setMoved();
            if (configuration.debug >= configuration.DEBUG_SPARSE) Log.i(TAG, "Sample is " + diff + " from AP, assume AP " + bestAP.getBSSID() + " has moved.");
        } else {
            bestAP.decMoved();
            for (int i=0; i<3; i++) {
    ...
    private synchronized void resetLocationCache() {
        if (configuration.debug >= configuration.DEBUG_VERBOSE) Log.i(TAG,"resetLocationCache");
    ...
    public synchronized Location ApLocation( String bssid ) {
        ...
        if (negative != null && negative.booleanValue()) {
            if (configuration.debug >= configuration.DEBUG_VERBOSE) Log.i(TAG,"ApLocation negative cache: "+(System.currentTimeMillis()-entryTime)+"ms");
            return null;
        }
        Location cached = queryResultCache.get(canonicalBSSID);
        if (cached != null) {
            if (configuration.debug >= configuration.DEBUG_VERBOSE) Log.i(TAG,"ApLocation positive cache: "+(System.currentTimeMillis()-entryTime)+"ms");
            return cached;
        }
        ...
        if (c != null) {
            if (c.getCount() == 1) {
                ...
                if (configuration.debug >= configuration.DEBUG_NORMAL) Log.i(TAG,"ApLocation time: "+(System.currentTimeMillis()-entryTime)+"ms");
                queryResultCache.put(canonicalBSSID, result);
                return result;
            }
            c.close();
        }
        if (configuration.debug >= configuration.DEBUG_VERBOSE) Log.i(TAG, "AP not found in database: " + bssid );
    ...
    private class apInfo {
        ...
        public apInfo(Cursor c) {
            if (configuration.debug >= configuration.DEBUG_VERBOSE) Log.i(TAG, "apInfo( Cursor c )" );
            if (!c.isLast()) {
                c.moveToNext();
                ...
                if (configuration.debug >= configuration.DEBUG_SPARSE) Log.i(TAG, "apInfo( Cursor c ) failed!" );
            }
        }
        ...
        public float calcRadius() {
            ...
            for (int i=0; i<3; i++) {
                ...
                if (configuration.debug >= configuration.DEBUG_VERBOSE) Log.i(TAG, "apInfo.calcRadius("+bssid+"): sample[" + i + "] = " + thisRadius );
                if (thisRadius > result) {
                    if (configuration.debug >= configuration.DEBUG_VERBOSE) Log.i(TAG, "apInfo.calcRadius("+bssid+"): sample[" + i + "] = " + sample[i].toString() );
                    result = thisRadius;
                }
            }
            if (configuration.debug >= configuration.DEBUG_NORMAL) Log.i(TAG, "apInfo.calcRadius(): " + result );
            return result;
        }
        ...
        public void insert() {
            if (configuration.debug >= configuration.DEBUG_SPARSE) Log.i(TAG, "apInfo.insert(): adding " + bssid + " to database" );
            radius = calcRadius();
        ...
        public void update() {
            ...
            if (changed) {
                if (configuration.debug >= configuration.DEBUG_SPARSE) Log.i(TAG, "apInfo.update(): updating " + bssid + " in database" );
